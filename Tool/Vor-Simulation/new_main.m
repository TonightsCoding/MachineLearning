...
sigmoid_function_y = SigmoidFunction(1000, -8, 8, -1, 1, 0);
...
activation_function_l1_all_neurons_gradient = 1;
...
gradient = activation_function_l1_all_neurons_gradient;
...
activation_function_l1_all_neurons_y = LinearFunction(resolution, gradient, -dnd, dnd, -dnd, dnd, -dnd, dnd, bias)';
...
for yi=1:1:(featureCnt)
   for xi=1:1:(featureCnt)
      ...
      [neuronNetTerms, neuronOutput] = GetNeuronOutput(inputs, weightVector, 0, 0, 'linear');
...
[neuronNetTerms, neuronOutput] = GetNeuronOutput(output_l1_all_neurons(featureMiddleStart:featureMiddleEnd, 1:end)', ones(1, featureCnt)', bias, dnd, 'sigmoid');
...
[neuronNetTerms, neuronOutput] = GetNeuronOutput(output_l1_all_neurons(1:end, featureMiddleStart:featureMiddleEnd), ones(featureCnt, 1), bias, dnd, 'sigmoid');
if (neuronOutput > threshold)
   plot(sum(neuronNetTerms), neuronOutput, 'o', 'color', 'g')
else
   plot(sum(neuronNetTerms), neuronOutput, 'o', 'color', 'r')
...
[neuronNetTerms1, neuronOutput1] = GetNeuronOutput(errorField1, ones(sz(1, 1), 1), bias, dnd, 'sigmoid');
[neuronNetTerms2, neuronOutput2] = GetNeuronOutput(errorField2, ones(sz(1, 1), 1), bias, dnd, 'sigmoid');
[neuronNetTerms3, neuronOutput3] = GetNeuronOutput(errorField3, ones(sz(1, 1), 1), bias, dnd, 'sigmoid');
[neuronNetTerms4, neuronOutput4] = GetNeuronOutput(errorField4, ones(sz(1, 1), 1), bias, dnd, 'sigmoid');
