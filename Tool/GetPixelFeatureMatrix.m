function pixelFeatureMatrix = GetPixelFeatureMatrix(pixelCnt, featureCnt, noise, featureMatrix, name)
   ...
   pixelFeatureMatrix = uint8(ones(pixelCntY_N, pixelCntX_N) .* 255);
   ...
   for iPixY = 0:(pixelCntY_N - 1)
      ...
      for iPixX = 0:(pixelCntX_N - 1)
         ...
         pixelFeatureMatrix(iPixY + 1, iPixX + 1) = featureMatrix(iFeatY + 1, iFeatX + 1);
      end
   end
   ...
   for iPixY = 1:pixelCntY_N
      for iPixX = 1:pixelCntX_N
         if (noiseMatrix(iPixY, iPixX) < noise)
            pixelFeatureMatrix(iPixY, iPixX) = 255 - pixelFeatureMatrix(iPixY, iPixX);
         end
      end
   end
   ...
   if (~isempty(name))
      imwrite(pixelFeatureMatrix, [name '.bmp']);
   end
end